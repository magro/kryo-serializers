...
/**
 * A {@link SerializerFactory} for creating instances of {@link FieldAnnotationRegardingSerializer} instances.
 *
 * @author <a href="mailto:rafael.wth@web.de">Rafael Winterhalter</a>
 */
public class FieldAnnotationRegardingSerializerFactory implements SerializerFactory {

    private final Collection<Class<? extends Annotation>> markedAnnotations;

    public FieldAnnotationRegardingSerializerFactory(Collection<Class<? extends Annotation>> markedAnnotations) {
    ...
    public Serializer makeSerializer(Kryo kryo, Class<?> aClass) {
        return new FieldAnnotationRegardingSerializer(kryo, aClass, markedAnnotations);
    }
}
